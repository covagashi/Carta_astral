...
def parse_carta_astral_pagina2(text):
    data = {}
         ...
         # Extraer Aspectario
    aspectario_match = re.search(r'Aspectario(.*?)(?=Distribución de Planetas|\Z)', text, re.DOTALL)
    if aspectario_match:
        aspectario_text = aspectario_match.group(1).strip()
        data['aspectario'] = parse_aspectario(aspectario_text)
    else:
        print("No se encontró la sección de Aspectario")
    ...
    if distribucion_match:
        ...
        print("No se encontró la sección de Distribución de Planetas")
...
def parse_aspectario(text):
    ...
    for i, line in enumerate(lines[1:], start=0):  # Empezamos desde la segunda línea
        parts = line.split()
        planeta1 = planetas[i]
        
        # Asumimos que cada línea debe tener tantos elementos como planetas hay
        # Si faltan elementos, los tratamos como espacios en blanco
        while len(parts) < len(planetas) + 1:
            parts.append('')
        
        for planetas2, aspecto in enumerate(parts[1:], start=0):
            if aspecto.strip() and aspecto != '-':
                
                aspecto_nombre = aspectos_map.get(aspecto.lower(), aspecto)
                if planeta1 != planetas2:  # Evitamos aspectos de un planeta consigo mismo
                    aspectario.append(f"{planeta1} {aspecto_nombre} {planetas2}")
